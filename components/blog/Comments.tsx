...
import { Send, Trash2, ChevronDown, ChevronUp, MessageSquare } from 'lucide-react';
...
const Comments: React.FC<CommentsProps> = ({ blogId, currentUser }) => {
  ...
  const renderComments = (parentId: string | null = null, depth: number = 0) => {
    ...
    return filteredComments.map((comment) => {
      ...
      const isExpanded = expandedComments[comment.id] || depth < 2;

      return (
        <div key={comment.id} className={`bg-gray-50 p-4 rounded-lg ${depth > 0 ? 'ml-6 mt-2' : 'mt-4'}`}>
          ...
          {replies.length > 0 && (
            <button
              ...
              className="mt-2 text-gray-500 flex items-center space-x-1 hover:text-gray-700 transition duration-200"
            >
              {isExpanded ? (
                <>
                ...
                <>
                  ...
                  <span>{replies.length}件の返信を表示</span>
                </>
              )}
            </button>
          )}
          {isExpanded && renderComments(comment.id, depth + 1)}
        </div>
      );
    });
  };
